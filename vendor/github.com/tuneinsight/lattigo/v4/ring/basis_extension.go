...
type BasisExtender struct {
	...
	// 0829: Add new params for Combined ModDown and Rescale.
	ringPq             []*Ring
	ringQlm1           []*Ring
	paramsPqtoQ        [][]ModupParams
	modDownparamsPqtoQ [][][]uint64
...
func NewBasisExtender(ringQ, ringP *Ring) *BasisExtender {
	...
	// 0829: append modulus in ringQ and ringP to construct series of ringPq:
	rescaleBudget := len(ringQ.Modulus) - 1 // length is based on the number of levels where one rescale is available.
	newParams.ringPq = make([]*Ring, rescaleBudget)
	newParams.ringQlm1 = make([]*Ring, rescaleBudget)
	newParams.modDownparamsPqtoQ = make([][][]uint64, rescaleBudget)
	newParams.paramsPqtoQ = make([][]ModupParams, rescaleBudget)
	newParams.buffPq = make([]*Poly, rescaleBudget)
	newParams.buffQlm1 = make([]*Poly, rescaleBudget)
	var err error
	for i := 0; i < rescaleBudget; i++ {
		// intialize ringPq
		Moduli_tmp := append(ringP.Modulus, ringQ.Modulus[i+1]) // Moduli = P*Q_{i+1}, this is for P*Q_{i+1) to Q_{i} ModDown.
		newParams.ringPq[i], err = NewRing(ringQ.N, Moduli_tmp)
		if err != nil {
			panic(err)
		}

		// initialize ringQlm-1
		newParams.ringQlm1[i], err = NewRing(ringQ.N, ringQ.Modulus[:i+1])
		if err != nil {
			panic(err)
		}

		// initialize paramsPqtoQ
		newParams.paramsPqtoQ[i] = make([]ModupParams, len(newParams.ringPq[i].Modulus))
		for j := range newParams.ringPq[i].Modulus {
			newParams.paramsPqtoQ[i][j] = GenModUpParams(newParams.ringPq[i].Modulus[:j+1], newParams.ringQlm1[i].Modulus) // ringQ.Modulus[:i+1] = Q_{i}
		}

		// initialize modDownparamsPqtoQ
		newParams.modDownparamsPqtoQ[i] = genModDownParams(newParams.ringQlm1[i], newParams.ringPq[i])

		// initialize buffs
		newParams.buffPq[i] = newParams.ringPq[i].NewPoly()
		newParams.buffQlm1[i] = newParams.ringQlm1[i].NewPoly()
	}
	// 0829
...
// ModDownQPtoQNTT reduces the basis of a polynomial.
// Given a polynomial with coefficients in basis {Q0,Q1....Ql} and {P0,P1...Pk},
// it reduces its basis from {Q0,Q1....Ql} and {P0,P1...Pk} to {Q0,Q1....Ql-1}
// and does a rounded integer division of the result by Qi*P.
// Inputs must be in the NTT domain.
func (be *BasisExtender) ModDownQPtoQlm1NTT(levelQ, levelP int, p1Q, p1P, p2Q *Poly) {
	levelQlm1 := levelQ - 1
	levelPq := levelP + 1
	if levelQlm1 < 0 {
		panic(errors.New("cannot do ModDownQPtoQlm1NTT due to out of modulus"))
	}

	ringQ := be.ringQlm1[levelQlm1]
	ringP := be.ringPq[levelQlm1]
	modDownParams := be.modDownparamsPqtoQ[levelQlm1]
	buffP := be.buffPq[levelQlm1]
	buffQ := be.buffQlm1[levelQlm1]

	// concatenate piQ's levelQ-th component and p1P to form p1Pq:
	// p1Pq :=

	// First we get the P basis part of p1 out of the NTT domain
	ringP.InvNTTLazyLvl(levelPq, p1P, buffP)

	// Then we target this P basis of p1 and convert it to a Q basis (at the "level" of p1) and copy it on the buffer.
	// The buffer is now the representation of the P basis of p1 but in basis Q (at the "level" of p1)
	// be.ModUpPtoQ(levelPq, levelQlm1, buffP, buffQ)
	be.ModUpPqtoQlm1(levelPq, levelQlm1, buffP, buffQ)

	// First, we switch back the buffer CRT array back to the NTT domain
	ringQ.NTTLazyLvl(levelQlm1, buffQ, buffQ)

	// Finally, for each level of p1 (and the buffer since they now share the same basis) we compute p2 = (P^-1) * (p1 - buff) mod Q
	for i := 0; i < levelQlm1+1; i++ {
		// Then for each coefficient we compute ((PQl)^-1) * (p1[i][j] - buff[i][j]) mod qi
		SubVecAndMulScalarMontgomeryTwoQiVec(buffQ.Coeffs[i], p1Q.Coeffs[i], p2Q.Coeffs[i], ringQ.Modulus[i]-modDownParams[levelPq][i], ringQ.Modulus[i], ringQ.MredParams[i])
	}
...
// Direct Copy of ModUpPtoQ, but Convert Mod P*Ql to Mod Ql-1
func (be *BasisExtender) ModUpPqtoQlm1(levelP, levelQ int, polP, polQ *Poly) {
	ModUpExact(polP.Coeffs[:levelP+1], polQ.Coeffs[:levelQ+1], be.ringPq[levelQ], be.ringQlm1[levelQ], be.paramsPqtoQ[levelQ][levelP])
}
