...
func Sigma_linearTransform_MultiThreadver2_dbg(params ckks.Parameters, sk *rlwe.SecretKey, rlk *rlwe.RelinearizationKey, galk *rlwe.RotationKeySet, ctMatrixA []*rlwe.Ciphertext, DSigmaLTs []ckks.LinearTransform, ColShiftPTs interface{}, d int, goroutineNum ...int) (ctOut [][]*rlwe.Ciphertext, err error) {
	DSigmaN1 := DSigmaLTs[len(DSigmaLTs)-1].N1
	var wg sync.WaitGroup
	var maxSubRoutes int
	if len(goroutineNum) > 0 {
		maxSubRoutes = goroutineNum[0]
	...
	for i := 0; i < len(ctMatrixA); i++ {
		...
		go func(i int) {
			defer wg.Done()
			evaluator := ckks.NewEvaluator(params, rlwe.EvaluationKey{Rlk: rlk, Rtks: galk})
			ctDSigmaA := evaluator.LinearTransform4ArithmeticSeqNew(ctMatrixA[i], DSigmaLTs[0])
			err = evaluator.Rescale(ctDSigmaA[0], params.DefaultScale(), ctDSigmaA[0])
			for j := 1; j < len(DSigmaLTs); j++ {
				ctDSigmaA[0] = evaluator.LinearTransform4ArithmeticSeqNew(ctDSigmaA[0], DSigmaLTs[j])[0]
				err = evaluator.Rescale(ctDSigmaA[0], params.DefaultScale(), ctDSigmaA[0])
			}
			switch ColShiftMask := ColShiftPTs.(type) {
			case []ckks.LinearTransform:
				now := time.Now()
				ctOut[i] = evaluator.ColShiftRestricted(ctDSigmaA[0], ColShiftMask, DSigmaN1, d)
				for j := range ctOut[i] {
					err = evaluator.Rescale(ctOut[i][j], params.DefaultScale(), ctOut[i][j])
...
func Sigma_linearTransform_MultiThreadver2_dbg_batch(params ckks.Parameters, sk *rlwe.SecretKey, rlk *rlwe.RelinearizationKey, galk *rlwe.RotationKeySet, ctMatrixA []*rlwe.Ciphertext, DSigmaLTs []ckks.LinearTransform, ColShiftPTs interface{}, d int, batchsize int, goroutineNum ...int) (ctOut [][]*rlwe.Ciphertext, err error) {
	DSigmaN1 := DSigmaLTs[len(DSigmaLTs)-1].N1
	g := batchsize
	var wg sync.WaitGroup
	var maxSubRoutes int
	...
	for i := 0; i < len(ctMatrixA); i++ {
		...
		go func(i int) {
			...
			case []ckks.LinearTransform:
				now := time.Now()
				ctOut[i] = evaluator.ColShiftRestricted_batch(ctDSigmaA[0], ColShiftMask, DSigmaN1, d, g)
				for j := range ctOut[i] {
					err = evaluator.Rescale(ctOut[i][j], params.DefaultScale(), ctOut[i][j])
...
func Tau_linearTransform_MultiThread_dbg(params ckks.Parameters, sk *rlwe.SecretKey, rlk *rlwe.RelinearizationKey, galk *rlwe.RotationKeySet, ctMatrixB []*rlwe.Ciphertext, DTauLTs []ckks.LinearTransform, d int, goroutineNum ...int) (ctOut [][]*rlwe.Ciphertext, err error) {
	...
	for i := 0; i < len(ctMatrixB); i++ {
		...
		go func(i int) {
			...
			var ctDTauB_RowShifted map[int]*rlwe.Ciphertext
			evaluator := ckks.NewEvaluator(params, rlwe.EvaluationKey{Rlk: rlk, Rtks: galk})
			ctDTauB := evaluator.LinearTransform4ArithmeticSeqNew(ctMatrixB[i], DTauLTs[0]) // ctDTauB := evaluator.LinearTransform4ArithmeticSeqNew(ctMatrixB[i], DTauLTs[len(DTauLTs)-1])
			err = evaluator.Rescale(ctDTauB[0], params.DefaultScale(), ctDTauB[0])
			for j := 1; j < len(DTauLTs); j++ { // j := len(DTauLTs) - 2; j >= 0; j--
				ctDTauB[0] = evaluator.LinearTransform4ArithmeticSeqNew(ctDTauB[0], DTauLTs[j])[0]
				err = evaluator.Rescale(ctDTauB[0], params.DefaultScale(), ctDTauB[0])
			}
...
func Tau_linearTransform_MultiThread_dbg_batch(params ckks.Parameters, sk *rlwe.SecretKey, rlk *rlwe.RelinearizationKey, galk *rlwe.RotationKeySet, ctMatrixB []*rlwe.Ciphertext, DTauLTs []ckks.LinearTransform, d int, batchsize int, goroutineNum ...int) (ctOut [][]*rlwe.Ciphertext, err error) {
	DTauN1 := DTauLTs[len(DTauLTs)-1].N1
	rots := make([]int, DTauN1)
	g := batchsize
	for i := range rots {
		rots[i] = (i + 1) * d * g // rots[i] = (i + 1) * d
	}
	var wg sync.WaitGroup
	var maxSubRoutes int
	if len(goroutineNum) > 0 {
	...
	for i := 0; i < len(ctMatrixB); i++ {
		...
		go func(i int) {
			...
			for k := 0; k < d-1; k += DTauN1 {
				...
				for key_batch := range ctDTauB_RowShifted {
					key := key_batch / g
					ctOut[i][k+key/d] = ctDTauB_RowShifted[key_batch].CopyNew()
				}
				ctDTauB[0] = ctDTauB_RowShifted[d*len(ctDTauB_RowShifted)*g].CopyNew()
			}
			fmt.Printf("Rowshift using %s\n", time.Since(now))
		}(i)
		if subrouteNum <= 0 {
