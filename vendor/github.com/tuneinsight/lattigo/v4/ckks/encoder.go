...
type Encoder interface {
	...
	// Slots Encoding with Montgomery On:
	EncodeMont(values interface{}, plaintext *rlwe.Plaintext, logSlots int)
	EncodeMontNew(values interface{}, level int, scale rlwe.Scale, logSlots int) (plaintext *rlwe.Plaintext)
...
// 0829: Add new method to encode:
func (ecd *encoderComplex128) EncodeMont(values interface{}, plaintext *rlwe.Plaintext, logSlots int) {
	ecd.Embed(values, logSlots, plaintext.Scale, true, plaintext.Value)
}

func (ecd *encoderComplex128) EncodeMontNew(values interface{}, level int, scale rlwe.Scale, logSlots int) (plaintext *rlwe.Plaintext) {
	plaintext = NewPlaintext(ecd.params, level)
	plaintext.Scale = scale
	ecd.EncodeMont(values, plaintext, logSlots)
	return
}
