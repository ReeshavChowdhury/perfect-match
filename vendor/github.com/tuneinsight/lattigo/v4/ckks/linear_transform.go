...
func (eval *evaluator) LinearTransformNew(ctIn *rlwe.Ciphertext, linearTransform interface{}) (ctOut []*rlwe.Ciphertext) {
	...
	case LinearTransform:
		now := time.Now() // debug test:
		minLevel := utils.MinInt(LTs.Level, ctIn.Level())
		eval.DecomposeNTT(minLevel, eval.params.PCount()-1, eval.params.PCount(), ctIn.Value[1], ctIn.IsNTT, eval.BuffDecompQP)
		fmt.Printf("DecomposeNTT uses %s\n", time.Since(now)) // debug test:
...
func (eval *evaluator) MultiplyByDiagMatrix(ctIn *rlwe.Ciphertext, matrix LinearTransform, BuffDecompQP []ringqp.Poly, ctOut *rlwe.Ciphertext) {
	...
	for k := range matrix.Vec {
		...
		if k == 0 {
			...
			// fmt.Printf("KeyswitchHoistedNoModDown and permute uses %s\n", time.Since(now)) // debug test
			...
			// fmt.Printf("Plaintext-Ciphertext Hoisted Mult uses %s\n", time.Since(now)) // debug test
...
func (eval *evaluator) MultiplyByDiagMatrixBSGS4ArithmeticSeq(ctIn *rlwe.Ciphertext, matrix LinearTransform, PoolDecompQP []ringqp.Poly, ctOut *rlwe.Ciphertext) {
	...
	// Computes the N2 rotations indexes of the non-zero rows of the diagonalized DFT matrix for the baby-step giant-step algorithm
	index, _, rotN2 := BsgsIndex4ArithmeticSeqGeneral(matrix.Vec, 1<<matrix.LogSlots, matrix.N1, matrix.CommonDiff)
...
func (eval *evaluator) LinearTransform4ArithmeticSeqNew(ctIn *rlwe.Ciphertext, linearTransform interface{}, shareInnerLoop ...bool) (ctOut []*rlwe.Ciphertext) {
	...
	case []LinearTransform:
		...
		// now := time.Now() // dbg testing
		eval.DecomposeNTT(minLevel, eval.params.PCount()-1, eval.params.PCount(), ctIn.Value[1], ctIn.IsNTT, eval.BuffDecompQP)
		// fmt.Printf("Decomposition consumes %s\n", time.Since(now)) // dbg testing
...
func GenLinearTransformBSGS4ArithmeticSeq(encoder Encoder, value interface{}, level int, scale rlwe.Scale, N1 int, commonDiff int, logSlots int) (LT LinearTransform) {
	...
	index, _, _ := BsgsIndex4ArithmeticSeqGeneral(value, slots, N1, commonDiff)
	...
	for j := range index {
		...
		for _, i := range index[j] {
			...
			// Sparse residuals + non-power-of-two N1: modular wrap of idxN1+idxN2
			// can miss a key; encode as zero diagonal (empty contribution).
			if v == nil {
				switch value.(type) {
				case map[int][]complex128:
...
// Notice that this function only support naive type of LinearTransformation implementation and do not support BSGS optimization.
// Also, the minLevel among the ctIn and LTs should be the level of ctIn.
func (eval *evaluator) ColShiftRestricted_batch(ctIn *rlwe.Ciphertext, LTs []LinearTransform, AvailableRots int, dimension int, batchsize int) (ctOut []*rlwe.Ciphertext) {
	// TODO: scale the rotation step by g, but since the key in any LT of LTs is already scaled by g, the key needed to be first rescaled by 1/g.
	AvailableKeys := AvailableRots
	TargetRots := len(LTs)
	d := dimension
	g := batchsize
	...
	params := eval.params
	eval.DecomposeNTT(levelQ, eval.params.PCount()-1, eval.params.PCount(), ctInBase.Value[1], ctInBase.IsNTT, tmp1BuffDecompQP)
	eval.AutomorphismHoisted(levelQ, ctInBase, tmp1BuffDecompQP, eval.params.GaloisElementForColumnRotationBy(((-d*g)+params.Slots())%params.Slots()), ctInNegBase)
	eval.DecomposeNTT(levelQ, eval.params.PCount()-1, eval.params.PCount(), ctInNegBase.Value[1], ctInNegBase.IsNTT, tmp2BuffDecompQP)
	...
	for t := 0; t < TargetRots; t += AvailableKeys {
		...
		for i, matrix := range LTs[t : t+end] {
			...
			for k_batch := range matrix.Vec {
				...
				if k == 0 {
					...
					galEl := eval.params.GaloisElementForColumnRotationBy(tmpk * g)
					rtk, generated := eval.Rtks.Keys[galEl]
					if !generated {
						panic("cannot MultiplyByDiagMatrix: switching key not available")
					}
					...
					if cnt == 0 {
						// keyswitch(c1_Q) = (d0_QP, d1_QP)
						ringQP.MulCoeffsMontgomeryLvl(levelQ, levelP, matrix.Vec[k_batch], tmp0QP, c0OutQP)
						ringQP.MulCoeffsMontgomeryLvl(levelQ, levelP, matrix.Vec[k_batch], tmp1QP, c1OutQP)
					} else {
						// keyswitch(c1_Q) = (d0_QP, d1_QP)
						ringQP.MulCoeffsMontgomeryAndAddLvl(levelQ, levelP, matrix.Vec[k_batch], tmp0QP, c0OutQP)
						ringQP.MulCoeffsMontgomeryAndAddLvl(levelQ, levelP, matrix.Vec[k_batch], tmp1QP, c1OutQP)
					}
		...
		eval.AutomorphismHoisted(levelQ, ctInBase, tmp1BuffDecompQP, eval.params.GaloisElementForColumnRotationBy(AvailableKeys*g), ctInBase)
		eval.AutomorphismHoisted(levelQ, ctInNegBase, tmp2BuffDecompQP, eval.params.GaloisElementForColumnRotationBy(AvailableKeys*g), ctInNegBase)
		eval.DecomposeNTT(levelQ, eval.params.PCount()-1, eval.params.PCount(), ctInBase.Value[1], ctInBase.IsNTT, tmp1BuffDecompQP)
		eval.DecomposeNTT(levelQ, eval.params.PCount()-1, eval.params.PCount(), ctInNegBase.Value[1], ctInNegBase.IsNTT, tmp2BuffDecompQP)
	}
...
// The BSGS approach is used (double hoisting with baby-step giant-step), which is faster than MultiplyByDiagMatrix
// for matrix with more than a few non-zero diagonals and uses significantly less keys.
func (eval *evaluator) MultiplyByDiagMatrixBSGS_flexkey(ctIn *rlwe.Ciphertext, matrix LinearTransform, PoolDecompQP []ringqp.Poly, ctOut *rlwe.Ciphertext, Rots_submap map[int][]int, sk *rlwe.SecretKey) {
	...
	// Pre-rotates ciphertext for the baby-step giant-step algorithm, does not divide by P yet
	rotN2_tmp := make([]int, 0)
	ctInRotQP_tmp := make(map[int]rlwe.CiphertextQP)
	for _, r := range rotN2 {
		if _, exists := Rots_submap[r]; exists {
			var ctpreRot_tmp *rlwe.Ciphertext
			for i, sub_r := range Rots_submap[r] {
				if i < len(Rots_submap[r])-1 {
					ctpreRot_tmp = eval.RotateNew(ctIn, sub_r)
				} else {
					ctpreRot_Tmp0 := ctpreRot_tmp.CopyNew().Value[0]
					decompRNS := eval.params.DecompRNS(eval.params.QCount()-1, eval.params.PCount()-1)
					tmpDmpQP := make([]ringqp.Poly, decompRNS)
					for i := 0; i < decompRNS; i++ {
						tmpDmpQP[i] = ringQP.NewPoly()
					}
					minLevel := utils.MinInt(matrix.Level, ctIn.Level())
					eval.DecomposeNTT(minLevel, eval.params.PCount()-1, eval.params.PCount(), ctpreRot_tmp.Value[1], ctpreRot_tmp.IsNTT, tmpDmpQP)
					ctInRotQP_tmp[r] = eval.RotateHoistedNoModDownNew(levelQ, []int{sub_r}, ctpreRot_Tmp0, tmpDmpQP)[sub_r]

				}
			}
		} else {
			rotN2_tmp = append(rotN2_tmp, r)
		}
	}
	ctInRotQP := eval.RotateHoistedNoModDownNew(levelQ, rotN2_tmp, ctInTmp0, eval.BuffDecompQP)
	for r, ct := range ctInRotQP_tmp {
		if _, exists := ctInRotQP[r]; exists {
			panic("conflict?")
		}
		ctInRotQP[r] = ct
	}
	...
	if debug {
		for r := range ctInRotQP {
			fmt.Printf("test RotateHoistedNoModDown:\n")
			ctOut_test := ctOut.CopyNew()
			eval.BasisExtender.ModDownQPtoQNTT(levelQ, levelP, ctInRotQP[r].Value[0].Q, ctInRotQP[r].Value[0].P, ctOut_test.Value[0])
			eval.BasisExtender.ModDownQPtoQNTT(levelQ, levelP, ctInRotQP[r].Value[1].Q, ctInRotQP[r].Value[1].P, ctOut_test.Value[1])
			ctOut_test.MetaData = ctIn.MetaData
			ctOut_test.Scale = ctIn.Scale
			// ring.CopyLvl(levelQ, ctOut.Value[0], ctInRotQP_tmp[r].Value[0].Q)
			// ring.CopyLvl(levelQ, ctOut.Value[1], ctInRotQP_tmp[r].Value[1].Q)
			decryptor := NewDecryptor(eval.params, sk)
			encoder := NewEncoder(eval.params)
			pt := decryptor.DecryptNew(ctOut_test)
	...
	for j := range index {
		...
		if j != 0 {
			...
			galEl := eval.params.GaloisElementForColumnRotationBy(j)
			if _, exists := Rots_submap[j]; exists {
				galEl = eval.params.GaloisElementForColumnRotationBy(Rots_submap[j][0])
			}
			...
			if _, exists := Rots_submap[j]; exists {
				c0OutQP_tmp := ringqp.Poly{Q: ctOut.CopyNew().Value[0], P: eval.BuffQP[5].CopyNew().Q}
				c1OutQP_tmp := ringqp.Poly{Q: ctOut.CopyNew().Value[1], P: eval.BuffQP[5].CopyNew().P}
				for idx := 1; idx < len(Rots_submap[j]); idx++ {
					if idx == 1 {
						ringQP.PermuteNTTWithIndexLvl(levelQ, levelP, cQP.Value[0], rotIndex, c0OutQP_tmp)
						ringQP.PermuteNTTWithIndexLvl(levelQ, levelP, cQP.Value[1], rotIndex, c1OutQP_tmp)
					}
					eval.BasisExtender.ModDownQPtoQNTT(levelQ, levelP, c0OutQP_tmp.Q, c0OutQP_tmp.P, c0OutQP_tmp.Q) // sum(phi(c0 * P + d0_QP))/P
					eval.BasisExtender.ModDownQPtoQNTT(levelQ, levelP, c1OutQP_tmp.Q, c1OutQP_tmp.P, c1OutQP_tmp.Q) // sum(phi(d1_QP))/P

					// ----------------------------------------------------------------------------
					if debug {
						fmt.Printf("test RotateHoistedNoModDown:\n")
						ctOut_test := ctOut.CopyNew()
						ring.CopyLvl(levelQ, c0OutQP_tmp.Q, ctOut_test.Value[0])
						ring.CopyLvl(levelQ, c1OutQP_tmp.Q, ctOut_test.Value[1])
						ctOut_test.MetaData = ctIn.MetaData
						ctOut_test.Scale = ctIn.Scale.Mul(matrix.Scale)
						decryptor := NewDecryptor(eval.params, sk)
						encoder := NewEncoder(eval.params)
						pt := decryptor.DecryptNew(ctOut_test)
						values := encoder.Decode(pt, eval.params.LogSlots())
					...
					ctpreRot_Tmp0 := c0OutQP_tmp.Q.CopyNew()
					decompRNS := eval.params.DecompRNS(eval.params.QCount()-1, eval.params.PCount()-1)
					tmpDmpQP := make([]ringqp.Poly, decompRNS)
					for i := 0; i < decompRNS; i++ {
						tmpDmpQP[i] = ringQP.NewPoly()
					...
					minLevel := utils.MinInt(matrix.Level, ctIn.Level())
					eval.DecomposeNTT(minLevel, eval.params.PCount()-1, eval.params.PCount(), c1OutQP_tmp.Q.CopyNew(), true, tmpDmpQP)
					ctOuterSubRot_tmp := eval.RotateHoistedNoModDownNew(levelQ, []int{Rots_submap[j][idx]}, ctpreRot_Tmp0, tmpDmpQP)[Rots_submap[j][idx]]
					c0OutQP_tmp = ringqp.Poly{Q: ctOuterSubRot_tmp.Value[0].Q, P: ctOuterSubRot_tmp.Value[0].P}
					c1OutQP_tmp = ringqp.Poly{Q: ctOuterSubRot_tmp.Value[1].Q, P: ctOuterSubRot_tmp.Value[1].P}

				}
				if cnt0 == 0 {
					ringQP.CopyLvl(levelQ, levelP, c0OutQP_tmp, c0OutQP)
					ringQP.CopyLvl(levelQ, levelP, c1OutQP_tmp, c1OutQP)
					// c0OutQP = c0OutQP_tmp.CopyNew()
					// c1OutQP = c1OutQP_tmp.CopyNew()
				} else {

					ringQP.AddNoModLvl(levelQ, levelP, c0OutQP, c0OutQP_tmp, c0OutQP)
					ringQP.AddNoModLvl(levelQ, levelP, c1OutQP, c1OutQP_tmp, c1OutQP)
				}
			} else {
				// Outer loop rotations
				if cnt0 == 0 {
		...
		if debug {
			...
			ringP.ReduceLvl(levelP, c1OutQP.P, c1OutQP.P)
			ctOut_test := ctOut.CopyNew()
			eval.BasisExtender.ModDownQPtoQNTT(levelQ, levelP, c0OutQP.Q, c0OutQP.P, ctOut_test.Value[0]) // sum(phi(c0 * P + d0_QP))/P
			eval.BasisExtender.ModDownQPtoQNTT(levelQ, levelP, c1OutQP.Q, c1OutQP.P, ctOut_test.Value[1]) // sum(phi(d1_QP))/P
			// ring.CopyLvl(levelQ, c0OutQP.Q, ctOut.Value[0])
			// ring.CopyLvl(levelQ, c1OutQP.Q, ctOut.Value[1])
			ctOut_test.MetaData = ctIn.MetaData
			ctOut_test.Scale = ctIn.Scale.Mul(matrix.Scale)
			decryptor := NewDecryptor(eval.params, sk)
			encoder := NewEncoder(eval.params)
...
// containing a single Ciphertext). A PtDiagMatrix is a diagonalized plaintext matrix constructed with an Encoder using
// the method encoder.EncodeDiagMatrixAtLvl(*).
func (eval *evaluator) LinearTransformNew_flexkey(ctIn *rlwe.Ciphertext, linearTransform interface{}, Rots_submap map[int][]int, sk *rlwe.SecretKey) (ctOut []*rlwe.Ciphertext) {
	...
	case LinearTransform:
		...
		if LTs.N1 == 0 {
			...
			eval.MultiplyByDiagMatrixBSGS_flexkey(ctIn, LTs, eval.BuffDecompQP, ctOut[0], Rots_submap, sk)
		}
