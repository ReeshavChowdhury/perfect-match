...
// GenGaloisKeyNewWithRNSDecomposition generates a rotation key whose gadget
// digits group r Q primes. This is an isolated coalesced-RNS prototype; the
// stock Lattigo key generator remains unchanged.
func (kgen KeyGenerator) GenGaloisKeyNewWithRNSDecomposition(galEl uint64, sk *SecretKey, r int) (gk *GaloisKey) {
	p := kgen.params.GetRLWEParameters()
	gk = &GaloisKey{
		EvaluationKey: EvaluationKey{GadgetCiphertext: *NewGadgetCiphertextWithRNSDecomposition(kgen.params, 1, p.MaxLevelQ(), p.MaxLevelP(), r)},
...
// GenGaloisKeysNewWithRNSDecomposition generates a set of prototype keys
// sharing one coalesced RNS digit width.
func (kgen KeyGenerator) GenGaloisKeysNewWithRNSDecomposition(galEls []uint64, sk *SecretKey, r int) (gks []*GaloisKey) {
	gks = make([]*GaloisKey, len(galEls))
	for i, galEl := range galEls {
		gks[i] = kgen.GenGaloisKeyNewWithRNSDecomposition(galEl, sk, r)
	}
	return
}
