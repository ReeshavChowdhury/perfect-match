...
func Quick_check_vector_full(params ckks.Parameters, sk *rlwe.SecretKey, ct *rlwe.Ciphertext) {
	values := DecryptDecode(params, sk, ct)
	realpart := Complex2Float(values)
	Print_vector_f64_full(realpart, 0)
}
...
func Quick_compare(params ckks.Parameters, sk *rlwe.SecretKey, ctA *rlwe.Ciphertext, ctB *rlwe.Ciphertext, prec float64) bool {
	if prec < 0 {
		panic(errors.New("prec < 0"))
	}

	vA := DecryptDecode(params, sk, ctA)
	vB := DecryptDecode(params, sk, ctB)
	realpartA := Complex2Float(vA)
	realpartB := Complex2Float(vB)
	for i := range realpartA {
		diff := math.Abs(realpartA[i] - realpartB[i])
		if diff > prec {
			fmt.Printf("difference: %f and %f at position %d\n", realpartA[i], realpartB[i], i)
			return false
		}
	}
	return true
