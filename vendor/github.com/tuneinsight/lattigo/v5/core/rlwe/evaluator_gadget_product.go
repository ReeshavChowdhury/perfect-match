...
func (eval Evaluator) gadgetProductMultiplePLazy(levelQ int, cx ring.Poly, gadgetCt *GadgetCiphertext, ct *Element[ringqp.Poly]) {
	...
	nbPi := gadgetCt.BaseRNSDecomposition
	if nbPi == 0 {
		nbPi = levelP + 1
	}
	// Evaluation keys are normally generated at MaxLevelQ, while the input
	// ciphertext may already have dropped levels. Only decompose the digits
	// intersecting the input's current Q basis. Iterating the key's full
	// max-level digit count makes the final digit start beyond levelQ and
	// causes DecomposeAndSplit to receive an empty (negative-length) interval.
	BaseRNSDecompositionVectorSize := (levelQ + 1 + nbPi - 1) / nbPi
	if BaseRNSDecompositionVectorSize > len(gadgetCt.Value) {
		BaseRNSDecompositionVectorSize = len(gadgetCt.Value)
	}
	...
	for i := 0; i < BaseRNSDecompositionVectorSize; i++ {
		...
		eval.DecomposeSingleNTT(levelQ, levelP, nbPi, i, cxNTT, cxInvNTT, c2QP.Q, c2QP.P)
