...
func MultiplyByDiagMatrixBSGS(eval EvaluatorForLinearTransformation, ctIn *rlwe.Ciphertext, matrix LinearTransformation, ctInPreRot map[int]*rlwe.Element[ringqp.Poly], opOut *rlwe.Ciphertext) (err error) {
	return multiplyByDiagMatrixBSGS(eval, ctIn, matrix, ctInPreRot, opOut, nil, true)
}

// MultiplyByDiagMatrixBSGSNoModDown evaluates one BSGS transform and leaves
// its final accumulator in QP. The inner ModDown before the giant key-switch
// is retained because GadgetProductLazy accepts a Q polynomial. This method
// is intended for summing independent giant-group results in QP before one
// global final ModDown.
func MultiplyByDiagMatrixBSGSNoModDown(eval EvaluatorForLinearTransformation, ctIn *rlwe.Ciphertext, matrix LinearTransformation, ctInPreRot map[int]*rlwe.Element[ringqp.Poly], opOut *rlwe.Element[ringqp.Poly]) (err error) {
	return multiplyByDiagMatrixBSGS(eval, ctIn, matrix, ctInPreRot, nil, opOut, false)
}

func multiplyByDiagMatrixBSGS(eval EvaluatorForLinearTransformation, ctIn *rlwe.Ciphertext, matrix LinearTransformation, ctInPreRot map[int]*rlwe.Element[ringqp.Poly], opOut *rlwe.Ciphertext, opOutQP *rlwe.Element[ringqp.Poly], finalModDown bool) (err error) {
	...
	if finalModDown {
		*opOut.MetaData = *ctIn.MetaData
		...
		opOutQP.MetaData = &rlwe.MetaData{}
		opOutQP.IsNTT = true
	}

	levelQ := utils.Min(ctIn.Level(), matrix.Level)
	if finalModDown {
		levelQ = utils.Min(opOut.Level(), levelQ)
	}
	levelP := params.MaxLevelP()
	...
	// Result in QP
	var c0OutQP, c1OutQP ringqp.Poly
	if finalModDown {
		c0OutQP = ringqp.Poly{Q: opOut.Value[0], P: BuffQP[5].Q}
		c1OutQP = ringqp.Poly{Q: opOut.Value[1], P: BuffQP[5].P}
	} else {
		c0OutQP = opOutQP.Value[0]
		c1OutQP = opOutQP.Value[1]
	}
