...
type GadgetCiphertext struct {
	...
	// BaseRNSDecomposition is the number of Q primes represented by one
	// gadget digit. The stock Lattigo value is levelP+1. A non-zero custom
	// value is used by the isolated coalesced-RNS prototype in folder 2.
	BaseRNSDecomposition int
...
func NewGadgetCiphertext(params ParameterProvider, Degree, LevelQ, LevelP, BaseTwoDecomposition int) *GadgetCiphertext {
	...
	return &GadgetCiphertext{BaseTwoDecomposition: BaseTwoDecomposition, BaseRNSDecomposition: LevelP + 1, Value: m}
}

// NewGadgetCiphertextWithRNSDecomposition allocates a gadget ciphertext whose
// RNS digits group r Q primes. It is intentionally separate from the stock
// constructor so existing evaluation keys retain exactly their old format.
func NewGadgetCiphertextWithRNSDecomposition(params ParameterProvider, Degree, LevelQ, LevelP, r int) *GadgetCiphertext {
	if r < 1 {
		panic("RNS decomposition size must be positive")
	}
	n := (LevelQ + 1 + r - 1) / r
	m := make(structs.Matrix[VectorQP], n)
	for i := range m {
		m[i] = make([]VectorQP, 1)
		m[i][0] = NewVectorQP(params, Degree+1, LevelQ, LevelP)
	}
	return &GadgetCiphertext{BaseRNSDecomposition: r, Value: m}
}
...
func (ct GadgetCiphertext) Equal(other *GadgetCiphertext) bool {
	return (ct.BaseTwoDecomposition == other.BaseTwoDecomposition) && (ct.BaseRNSDecomposition == other.BaseRNSDecomposition) && cmp.Equal(ct.Value, other.Value)
}
...
func (ct GadgetCiphertext) CopyNew() (ctCopy *GadgetCiphertext) {
	return &GadgetCiphertext{BaseTwoDecomposition: ct.BaseTwoDecomposition, BaseRNSDecomposition: ct.BaseRNSDecomposition, Value: ct.Value.CopyNew()}
}

// BinarySize returns the serialized size of the object in bytes.
func (ct GadgetCiphertext) BinarySize() (dataLen int) {
	return 16 + ct.Value.BinarySize()
}
...
func (ct GadgetCiphertext) WriteTo(w io.Writer) (n int64, err error) {
	...
	case buffer.Writer:
		...
		n += inc
		if inc, err = buffer.WriteAsUint64[int](w, ct.BaseRNSDecomposition); err != nil {
			return n + inc, err
		}
		n += inc
...
func (ct *GadgetCiphertext) ReadFrom(r io.Reader) (n int64, err error) {
	...
	case buffer.Reader:
		...
		n += inc
		if inc, err = buffer.ReadAsUint64[int](r, &ct.BaseRNSDecomposition); err != nil {
			return n + inc, err
		}
		n += inc
...
func AddPolyTimesGadgetVectorToGadgetCiphertext(pt ring.Poly, cts []GadgetCiphertext, ringQP ringqp.Ring, buff ring.Poly) (err error) {
	...
	nbPi := cts[0].BaseRNSDecomposition
	if nbPi == 0 {
		nbPi = levelP + 1
	}
