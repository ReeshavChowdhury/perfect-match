...
func (decomposer *Decomposer) DecomposeAndSplit(levelQ, levelP, nbPi, BaseRNSDecompositionVectorSize int, p0Q, p1Q, p1P Poly) {
	...
	if decompLvl < 0 {
		...
		// The stock path caches constants for nbPi=len(P). The isolated
		// coalesced-RNS prototype may request another digit width, so build
		// the corresponding constants on demand. This is intentionally a
		// correctness-first path; production use should cache these values.
		var MUC ModUpConstants
		if nbPi == levelP+1 && nbPi-2 < len(decomposer.ModUpConstants) {
			...
			Pi := append([]uint64{}, Q...)
			if ringP != nil {
				Pi = append(Pi, ringP.ModuliChain()...)
			}
			MUC = GenModUpConstants(Q[start:end], Pi)
		}
